...
!==============================================
! 6. CALCULS DE DONNEES COMPLEMENTAIRES
!==============================================

!> Open and read file dp_CIO
...
if ( ippmod(ielion).ge.1 ) then
  ...
  do ii = 1, 5
    read(impfpp,*)
  enddo
  read(impfpp,*,err=999,end=999) modcond, nbz
  do ii = 1, 2
    read(impfpp,*)
  enddo
  read(impfpp,*,err=999,end=999) alphaci, qelem, zpl, znl, zps, zns
  do iz = 1, nbz
    do ii=1,4
      read(impfpp,*)
    enddo
    read(impfpp,*,err=999,end=999) zone(iz)
    read(impfpp,*)
    read(impfpp,*)
    read(impfpp,*,err=999,end=999) dpl(iz), dnl(iz),dl(iz) , dps(iz), dns(iz), ds(iz), dsln(iz), dslp(iz)
    read(impfpp,*)
    read(impfpp,*)
    read(impfpp,*,err=999,end=999) npl(iz), nnl(iz), nl(iz), nps(iz), nns(iz), ns(iz), nsln(iz), nslp(iz)
    read(impfpp,*)
    read(impfpp,*)
    read(impfpp,*,err=999,end=999) kdl(iz), krl(iz), kds(iz), krs(iz), kfsln(iz), krsln(iz), kfslp(iz), krslp(iz)
    read(impfpp,*)
    read(impfpp,*)
    read(impfpp,*,err=999,end=999) epsilr(iz), rhomas(iz)
    enddo
endif
