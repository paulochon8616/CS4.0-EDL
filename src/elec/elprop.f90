...
! ---> Divergence of AL+ migration term within the liquid

  call add_property_field('idivjpl', 'divJPL', idivjpl)

! ---> Divergence of BL- migration term within the liquid

  call add_property_field('idivjnl', 'divJNL', idivjnl)

! ---> Divergence of CS+ migration term within the solid

  call add_property_field('idivjps', 'divJPS', idivjps)

! ---> Divergence of DS- migration term within the solid

  call add_property_field('idivjns', 'divJNS', idivjns)

! ---> Current density

  do idimve = 1, ndimve
  write(f_name,  '(a11,i1)')  'curr_densit', idimve
  write(f_label, '(a7,i1.1)') 'ij_curr', idimve
    call add_property_field(f_name, f_label, ij(idimve))
  enddo

! ---> Displacement current density

  do idimve = 1, ndimve
  write(f_name,  '(a11,i1)')  'curr_deplac', idimve
  write(f_label, '(a7,i1.1)') 'ij_depl', idimve
    call add_property_field(f_name, f_label, ijdep(idimve))
  enddo

! ---> Electric field

  do idimve = 1, ndimve
  write(f_name,  '(a11,i1)')  'elec_field_', idimve
  write(f_label, '(a7,i1.1)') 'elfield', idimve
    call add_property_field(f_name, f_label, ichele(idimve))
  enddo
...
! ---> Flux [AL+]
  ...
  do idimve = 1, ndimve
  write(f_name,  '(a11,i1)')  'igama_alp__', idimve
  write(f_label, '(a7,i1.1)') 'gam_alp', idimve
    call add_property_field(f_name, f_label, igamapl(idimve))
  enddo

! ---> Flux [BL-]

  do idimve = 1, ndimve
  write(f_name,  '(a11,i1)')  'igama_bln__', idimve
  write(f_label, '(a7,i1.1)') 'gam_bln', idimve
    call add_property_field(f_name, f_label, igamanl(idimve))
  enddo

! ---> Flux [CS+]

  do idimve = 1, ndimve
  write(f_name,  '(a11,i1)')  'igama_csp__', idimve
  write(f_label, '(a7,i1.1)') 'gam_csp', idimve
    call add_property_field(f_name, f_label, igamaps(idimve))
  enddo

! ---> Flux [CS-]

  do idimve = 1, ndimve
  write(f_name,  '(a11,i1)')  'igama_dsn__', idimve
  write(f_label, '(a7,i1.1)') 'gam_dsn', idimve
    call add_property_field(f_name, f_label, igamans(idimve))
  enddo
